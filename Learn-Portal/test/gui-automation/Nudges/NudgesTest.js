...
describe('Learn Portal - Nudges module cases for Neo user - after class start 0-15 min', () => {
...
describe('Learn Portal - Nudges module cases for Neo user - after class start', () => {
...
describe('Learn Portal - Nudges module cases for Neo user - 30-15 min', () => {
    ...
    it('334734 TC_13 Verify the Regular class nudge for paid user not the masterclasses',async () => {
        await ProfilePage.changeCohortDetail(dashboardData.cohortDetailsForBookTrialValidation.cohortDetails[0][5], 'neo')
         await NudgesPage.preVerificationOfNudgeWindow()
        allure.startStep("Validate nudges is present or not")
        try{await NudgesPage.btnNudges.waitForDisplayed({timeout:5000})}
    ...
    it.skip('334719 TC_14 Verify the nuges feature is also applicable for MLP and 2k users',async () => {
        await browser.reloadSession()
        await ProfilePage.changeCohortDetail(dashboardData.cohortDetailsForBookTrialValidation.cohortDetails[0][5], 'MLP')
         await NudgesPage.preVerificationOfNudgeWindow()
        allure.startStep("Validate nudges is present or not")
        try{await NudgesPage.btnNudges.waitForDisplayed({timeout:5000})}
        catch{}
